 ...
 */
OU.activity.RecursiveInsertionSort = function (data, instance, controller) {
	 ...
	 * </ul>
	 */
	OU.activity.RecursiveInsertionSort.prototype.canvasView = function () {
		this.action = "other";
	...
	OU.activity.RecursiveInsertionSort.prototype.setAnimationSteps = function (steps) {
	...
	OU.activity.RecursiveInsertionSort.prototype.initDisplacements = function () {
		var i,
			inc,
	...
	OU.activity.RecursiveInsertionSort.prototype.initControls = function () {
		var self = this,
			bH = OU.controlHeight,
			btnSpace = 100 / 5,
	...
	OU.activity.RecursiveInsertionSort.prototype.pause = function () {
		this.togglePause();
	...
	OU.activity.RecursiveInsertionSort.prototype.togglePause = function () {
	...
	OU.activity.RecursiveInsertionSort.prototype.resetAnimation = function () {
		this.sortBtn.enable();
		...
		this.unsortedIndex = this.bars.length;
		this.sortedIndex = -1;
	...
	OU.activity.RecursiveInsertionSort.prototype.initVisualisation = function () {
	...
	OU.activity.RecursiveInsertionSort.prototype.initBarHeight = function () {
	...
	OU.activity.RecursiveInsertionSort.prototype.initBars = function () {
	...
	OU.activity.RecursiveInsertionSort.prototype.initBarGradients = function () {
	...
	OU.activity.RecursiveInsertionSort.prototype.getRGB = function (h, s, v) {
	    var r, g, b, i, f, p, q, t;
...
            break;
	 ...
	 * The main renderer.
	 */
	OU.activity.RecursiveInsertionSort.prototype.render = function () {
		this.drawVisualisation(); // draw the bars in current state
	};

	OU.activity.RecursiveInsertionSort.prototype.drawVisualisation = function () {
		...
		ctx.fillStyle = this.data.highlight; // draw the highlight for the sorted side
		ctx.fillRect(0, 0, barSpace * (this.sortedIndex), this.horizon + halfPad);
		...
		for (i = barCount; i >= 0; i--) {  // for each bar from right to left
			...
			if (this.animating > -1 && this.sorted) { // if still animating but all sorted
				...
				this.drawBar(ctx, this.bars[i], false, i >= this.unsortedIndex); // draw bar without glow
	...
	// draw individual bar
	OU.activity.RecursiveInsertionSort.prototype.drawBar = function (ctx, bar, shadow, fade) {
	...
	OU.activity.RecursiveInsertionSort.prototype.getBarY = function (height) {
		return this.horizon - height;
	};


	// draw a single arrow
	OU.activity.RecursiveInsertionSort.prototype.drawArrow = function (ctx, bar, color) {
	...
	OU.activity.RecursiveInsertionSort.prototype.start = function () {
		// init all the vars to starting values this.sortBtn.disable();
	...
	OU.activity.RecursiveInsertionSort.prototype.recordSort = function () {
		...
		this.replay.push({
			"action": "other",
			"vo": "begin",
			"unsorted": this.bars.length
		});

		this.sortRecursive(list);

		if (test.toString() !== list.toString()) {
			console.err("Sort fail!");
		}
	};

	OU.activity.RecursiveInsertionSort.prototype.sortRecursive = function (list) {
		var key,
			index;
		...
		this.replay.push({
			...
			"last": list.length,
			"sorted": list.length,
			"unsorted": list.length + 1
		});
		this.replay.push({
			"action": "other",
			"vo": "unwind",
			"sorted": list.length + 1,
			"unsorted": list.length + 1
		});

		list.splice(index + 1, 0, key);
	};

	// the timer loop to create a timeline for replay and animation
	OU.activity.RecursiveInsertionSort.prototype.renderLoop = function () {
	...
	OU.activity.RecursiveInsertionSort.prototype.step = function () {
	...
	OU.activity.RecursiveInsertionSort.prototype.replaySort = function () {
		var swap,
			last,
			replay,
			temp,
			...
			vo,
			self = this,
           almostSorted = this.bars.length - 1;
		...
		if (!this.dead && this.replay.length > 0) { // if not finished and not last step
			...
			if (typeof replay.unsorted !== "undefined") {
				this.unsortedIndex = replay.unsorted; // the left bar being compared
			}
			if (typeof replay.sorted !== "undefined") {
				this.sortedIndex = replay.sorted;
				this.unsortedIndex = replay.unsorted; // the left bar being compared
			}
			if (typeof replay.swap !== "undefined") {
				swap = replay.swap;
				last = replay.last;
			}

			buffer = [];
			if (swap > -1) { // if they were swapped
				temp = this.bars[last]; // swap the local vars
				buffer.push({
					"action": "save",
					"i": last
				});
			...
			for (j = 0; j < length; j++) {
				this.replay.unshift(buffer.pop());
			}
			...
			case "save":
				...
				this.animating = 0;
				if (this.unsortedIndex === 3) {
					this.playIt("save");
				}
				break;

			case "movedown":
				this.comparitor = replay.i;
			...
			case "other":
				switch (vo) {
				case "recurse":
					vo = (this.unsortedIndex >= 9 || this.unsortedIndex === 1) ?
							vo + this.unsortedIndex : "undefined";
					break;
				case "unwind":
					vo = (this.unsortedIndex >= 2 && this.unsortedIndex <= 2) ?
							vo + this.unsortedIndex : "undefined";
					break;
				case "sort":
					vo = (this.unsortedIndex >= 1 && this.unsortedIndex <= 4) ?
							vo + this.unsortedIndex : "undefined";
					break;
				}
				if (typeof vo !== "undefined" && vo !== "undefined") {
					this.playIt(vo);
				}
				break;
			}
            
           if (this.sortedIndex === almostSorted && this.unsortedIndex === almostSorted && replay.unsorted === almostSorted) {
               this.playIt("sorted");
           }

			// set global var for swapped
			this.swapIndex = swap;
		...
		if (this.audioPlay && this.audioToPlay && (this.stepCount < this.goFast || this.sorted)) { // audio checkbox selected
	...
	OU.activity.RecursiveInsertionSort.prototype.playIt = function (part) {
	...
	OU.activity.RecursiveInsertionSort.prototype.speedUp = function () {
	...
	OU.activity.RecursiveInsertionSort.prototype.animatingFinished = function () {
		this.step(); // do one more step
	};


	// audio checkbox clicked
	OU.activity.RecursiveInsertionSort.prototype.toggleAudio = function () {
	 ...
	 */
	OU.activity.RecursiveInsertionSort.prototype.accessibleView = function () {
		OU.activity.RecursiveInsertionSort.superClass_.accessibleView.call(this); // call the superclass method is you want to extend it, remove this line to override the method instead
	 ...
	 * </ul>
	 */
	OU.activity.RecursiveInsertionSort.prototype.resize = function () {
		OU.activity.RecursiveInsertionSort.superClass_.resize.call(this); // call the superclass resize
	...
	OU.activity.RecursiveInsertionSort.prototype.playAudio = function (audioFile) {
		if (this.audioPlay) {
	 ...
	 */
	OU.activity.RecursiveInsertionSort.prototype.remove = function () {
		OU.activity.RecursiveInsertionSort.superClass_.remove.call(this); // call the superclass remove
...
OU.inherits(OU.activity.RecursiveInsertionSort, OU.util.Activity);