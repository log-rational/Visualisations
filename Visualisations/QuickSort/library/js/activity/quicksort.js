/**
 * @fileOverview QuickSort - A template for activities that extend the OU.util.Activity class
 *
...
/**
 * @class QuickSort - A template for activities that extend the OU.util.Activity class
 ...
 */
OU.activity.QuickSort = function (data, instance, controller) {
     ...
     * </ul>
     */
    OU.activity.QuickSort.prototype.canvasView = function () {
        this.fastTo = -1; // debugging tool - go fast to this step when audio off, -1 means ignore
        this.rand = false;
        this.firstgo = true;
    ...
    OU.activity.QuickSort.prototype.setAnimationSteps = function (steps) {
    ...
    OU.activity.QuickSort.prototype.initDisplacements = function () {
		var i, inc,
    ...
    OU.activity.QuickSort.prototype.initControls = function () {
        var self = this,
			bH = OU.controlHeight,
			btnSpace = 100 / 6,
        ...
        this.randBtn = Utils.newButton( // init the audio rand chechbox button
            (btnSpace * 4 + xPad) + "%",
            (btnTop) + "%", //left,top
            "Random",
            function () {
    ...
    OU.activity.QuickSort.prototype.togglePause = function () {
    ...
    OU.activity.QuickSort.prototype.pause = function () {
        this.togglePause();
    ...
    OU.activity.QuickSort.prototype.resetAnimation = function () {
        this.sortBtn.enable();
    ...
    OU.activity.QuickSort.prototype.initVisualisation = function () {
    ...
    OU.activity.QuickSort.prototype.initBarHeight = function () {
    ...
    OU.activity.QuickSort.prototype.initBars = function () {
        var hue1 = this.hueStart, // paint bars with colours through the rainbow so start at red
			...
			firstPositions = [4, 12, 5, 11, 0, 2, 9, 6, 7, 8, 3, 10, 1], // keep note of order positions available for the bars ie 1st to Nth
    ...
    OU.activity.QuickSort.prototype.initBarGradients = function () {
    ...
    OU.activity.QuickSort.prototype.getRGB = function (h, s, v) {
        var r, g, b, i, f, p, q, t;
...
            break;
     ...
     * The main renderer.
     */
    OU.activity.QuickSort.prototype.render = function () {
        this.drawVisualisation(); // draw the bars in current state
    };

    OU.activity.QuickSort.prototype.drawBox = function (ctx, left, right, PAD, halfPad, barSpace) {
        if (right > 0 && left !== right) {
            ctx.fillRect(barSpace * (left) + halfPad - halfPad / 2, 0, barSpace * (right - left) - PAD + halfPad, this.horizon + halfPad);
		}

    };

    OU.activity.QuickSort.prototype.drawVisualisation = function () {
        var ctx = this.visualisationLayer.context, // get the graphics context
			...
			barSpace = w / this.bars.length, // the space needed for each bar including padding
        ...
        if (!this.sorted) {
            if (this.vo === "valuesLeft") {
                ctx.fillStyle = this.data.highlight;
                this.drawBox(ctx, this.first, this.pivot, PAD, halfPad, barSpace);
            } else if (this.vo === "valuesRight") {
                ctx.fillStyle = 'grey';
                this.drawBox(ctx, this.pivot, this.pivot + 1, PAD, halfPad, barSpace);
                ctx.fillStyle = this.data.highlight;
                this.drawBox(ctx, this.pivot + 1, this.bars.length, PAD, halfPad, barSpace);
            } else {
                ctx.fillStyle = this.data.highlight;
                if (this.first > 0) {
                    this.drawBox(ctx, 0, this.first, PAD, halfPad, barSpace);
				}
                this.drawBox(ctx, this.last + 1, this.bars.length, PAD, halfPad, barSpace);
            }
        }
        // draw bars
        for (i = barCount; i >= 0; i--) {  // for each bar from right to left
            ...
            if (this.animating > -1 && this.sorted) { // if still animating but all sorted
                ...
                highlight = (i === this.left && this.leftSelected) || (i === this.right && this.rightSelected);
                this.drawBar(ctx, this.bars[i], highlight, highlight); // draw bar without glow
            }
        }

        if (!this.sorted && this.vo !== "intro1") {
            arrowW = this.bars[0].width;
            rightX = this.bars[this.rightMark].x;
            leftX = this.bars[this.leftMark].x;
            pivotX = this.bars[this.pivot].x;
        }
        ...
        if (!this.sorted) {
            if (this.vo !== "intro1") {
                this.drawArrow(ctx, rightX, arrowW, this.getRGB(this.hueStart, 1, 1)); // draw arrow under right bar in stop color
                this.drawArrow(ctx, leftX, arrowW, 'orange', 'medium'); // draw arrow under left bar in start color
                this.drawArrow(ctx, pivotX, arrowW, this.getRGB(this.hueEnd, 1, 1), 'small'); // draw arrow under right bar in stop color
                if (this.vo !== "intro2") {
                    this.drawLine(ctx, this.bars[this.pivot]);
                }
            }
        }
    ...
    // draw individual bar
    OU.activity.QuickSort.prototype.drawLine = function (ctx, bar) {
        var w = this.visualisationLayer.w, // get width
			height = bar.height * this.barHeightUnit, // get the drawing height from height of bar and the basic unit
        ...
        ctx.strokeStyle = "red"; // get fill color
        ///ctx.setLineDash([5]);

        ctx.moveTo(0, y); // top/centre point of vertically up arrow

        ctx.lineTo(bar.x, y); // down first slope to right point of arrow head
        ctx.stroke();
        ctx.moveTo(bar.x + bar.width, y); // down first slope to right point of arrow head
        ctx.lineTo(w, y); // down first slope to right point of arrow head
        ctx.stroke();
    ...
    // draw individual bar
    OU.activity.QuickSort.prototype.drawBar = function (ctx, bar, shadow, selected) {
    ...
    OU.activity.QuickSort.prototype.getBarY = function (height) {
        return this.horizon - height;
    };

    // draw a single arrow
    OU.activity.QuickSort.prototype.drawArrow = function (ctx, x, width, color, size) {
        var thickness = (width) / 10,  // half thickness of the stem
        ...
        left = x + padX; // extreme left of arrowhead
        centre = x + width / 2; // centre point of arrow and location of tip
        right = x + width - padX; // extreme right of arrowhead
    ...
    OU.activity.QuickSort.prototype.start = function () {
    ...
    OU.activity.QuickSort.prototype.recordSort = function () {
        ...
        this.quickSortHelper(list, 0, list.length - 1, 0, "none");
    };

    OU.activity.QuickSort.prototype.quickSortHelper = function (list, first, last, level, side) {
        if (first < last) {
            var splitPoint = this.partition(list, first, last, level, side);
            this.quickSortHelper(list, first, splitPoint - 1, level + 1, "left");
            this.quickSortHelper(list, splitPoint + 1, last, level + 1, "right");
        }
        if (this.firstgo && this.count === 4 && first === 2 && last === 2) {
            this.replay.push({
                "id": 0,
                "return": true,
                "info": {
    ...
    OU.activity.QuickSort.prototype.partition = function (list, first, last, level, side) {
        this.count++;
        var pivot = first,
        ...
        while (!done) {
            ...
            if (list[rightMark] < pivotValue && rightMark > leftMark) {
                this.replay.push({
                    "id": 6,
                    "info": info,
                    "swap": "false",
            ...
            if (rightMark <= leftMark) {
                ...
                this.swap(list, leftMark, rightMark);
                this.replay.push({
                    "id": 8,
                    "info": info,
                    "swap": "leftright",
        ...
        if (rightMark > pivot) {
            this.swap(list, pivot, rightMark);
            this.replay.push({
                "id": 9,
                "info": info,
                "swap": "pivot",
    ...
    OU.activity.QuickSort.prototype.swap = function (list, left, right) {
        var temp = list[left];
    ...
    OU.activity.QuickSort.prototype.renderLoop = function () {
    ...
    OU.activity.QuickSort.prototype.step = function () {
        ...
        if (!this.dead && this.animating === -1 && this.swapped === "false") {
        ...
        if (this.animating === -1 && this.swapped !== "false") {
            this.replaySort(); // replay the sorting step
        ...
        if (this.animating === 1) {
            ...
            if (this.pauseCount <= 0) { // when it reaches zero
                this.pauseCount = this.pauseTime; // reset the pausecount
                this.drawVisualisation(); // redraw the bars
            }
        }
    ...
    OU.activity.QuickSort.prototype.replaySort = function () {
		var audioToPlay = true,
			self = this,
			replay,
			leftMark,
			rightMark,
        ...
        if (!this.dead && this.stepCount < this.replay.length) { // if not finished and not last step
            ...
            leftMark = replay.leftMark; // the left bar being compared
            rightMark = replay.rightMark; // the right bar
            pivot = replay.pivot;
            swap = replay.swap; // whether they were swapped
            vo = replay.vo;
            left = leftMark;
            right = rightMark;
            this.first = replay.info.first;
            this.last = replay.info.last;
            this.count = replay.info.count;
            this.level = replay.info.level;
            this.side = replay.info.side;
            this.leftSelected = replay.leftSelected;
            this.rightSelected = replay.rightSelected;
            ...
            if (swap !== "false") { // if they were swapped
            ...
            if (this.firstgo && this.stepCount >= 3 && this.stepCount <= 47) {
                vo = "firstgo" + (this.stepCount);
            }
            ...
            this.pivot = parseInt(pivot, 10);
            this.swapped = swap;
            this.max = replay.max;

            // show the correct text
            if (this.stepCount > this.fastTo && this.stepCount < this.goFast) { // if not going fast yet then display the comparision text
                this.textDiv.div.innerHTML = this.data[vo].text; // set the text div
                this.audioElement.setAttribute('src', this.dataDir + this.data[vo].audio); // set the is sorted audio to no
                this.pauseTime = this.data[vo].pause; // pause for audio
            } else { // go fast now
                ...
                this.pauseTime = this.data.speedPause;
            }
        ...
        if (this.audioPlay && audioToPlay) { // audio checkbox selected
            ...
            this.pauseTime = this.stepCount > this.fastTo && this.stepCount < this.goFast ? this.pauseTimeMute : this.data.speedPause; // set the pause - none if sped up
        }
    };

    OU.activity.QuickSort.prototype.playIt = function (part) {
    ...
    OU.activity.QuickSort.prototype.speedUp = function () {
    ...
    OU.activity.QuickSort.prototype.animatingFinished = function () {
        this.step(); // do one more step
    };

    // audio checkbox clicked
    OU.activity.QuickSort.prototype.toggleAudio = function () {
     ...
     */
    OU.activity.QuickSort.prototype.accessibleView = function () {
        OU.activity.QuickSort.superClass_.accessibleView.call(this); // call the superclass method is you want to extend it, remove this line to override the method instead
     ...
     * </ul>
     */
    OU.activity.QuickSort.prototype.resize = function () {
        OU.activity.QuickSort.superClass_.resize.call(this); // call the superclass resize
    ...
    OU.activity.QuickSort.prototype.playAudio = function (audioFile) {
        if (this.audioPlay) {
     ...
     */
    OU.activity.QuickSort.prototype.remove = function () {
        OU.activity.QuickSort.superClass_.remove.call(this); // call the superclass remove
...
OU.inherits(OU.activity.QuickSort, OU.util.Activity);